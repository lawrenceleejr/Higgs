C         Last modification on July 31 2014 by M.S.
C ==================================================================
...
c     WRITE(NSB,20)AMSM,SMBRG,SMBRGA,SMBRZGA,SMBRW,SMBRZ,SMWDTH,SMBRB*SMBRL*SMBRM*SMBRS*SMBRC*SMBRG*SMBRGA*SMBRZGA*SMBRW*SMBRZ,SMBRB*SMBRL*SMBRM*SMBRS*SMBRT*SMBRC*SMBRG*SMBRGA*SMBRZGA*SMBRW*SMBRZ
...
c        XFAC00 = CDABS(CAT0+CAB0+CAC0+CATP+CABP)**2*FQCD
         XFAC00= DREAL(DCONJG(CAT+CAB+CAC+CATP+CABP)
     .                     * (CAT0+CAB0+CAC0+CATP+CABP))*FQCD
        ELSE
...
C  H ---> TAU TAU
     ...
     .    *SM4FACF
      ENDIF
      ENDIF
c      write(6,*)'tau tau: ',AMH,ELW(AMH,AMTAU,-1.D0,7.D0),HFFSELF(AMH)
C  H --> SS
      IRAT = 0
      RATCOUP = 1
     ...
     .    *QCDH(RMS**2/AMH**2)
     .    * ((CPS-1)
     .         +(1+ELW(AMH,RMS,-1.D0/3.D0,7.D0)*(1+XQCD))
     .       *HFFSELF(AMH))
      ENDIF
...
C  H --> CC
     ...
     .    *QCDH(RMC**2/AMH**2)
     .    * ((CPC-1)
     .       +(1+ELW(AMH,RMC,2.D0/3.D0,7.D0)*(1+XQCD))
     .       *HFFSELF(AMH))
     .   + DCC
      ENDIF
...
C  H --> BB :
     ...
     .     *QCDH(RMB**2/AMH**2)
     .     * ((CPB-1)
     .        +(1+ELW(AMH,RMB,-1.D0/3.D0,1.D0)*(1+XQCD))
     .        *HFFSELF(AMH))
     .    + DBB
       ENDIF
...
c    .        ELW(AMH,AMB,-1.D0/3.D0,1.D0),HFFSELF(AMH)
...
c    .           dsqrt(xfac)/8
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
c       write(6,*)'H gamma gamma: ',CAT,CAW,CAT+CAW,(CAT+CAW)*9
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
